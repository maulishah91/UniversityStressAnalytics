...
class ConvNetModel():
    def __init__(self, x_len, word_embeddings, conv_Ws, conv_bs, fc_W, fc_b):
        # conv_W (100, 1, filter height, 400)
        # conv_b (100)
        # fc_W (300,3)
        # fc_b (300)
        ...
        for i in xrange(len(self.filter_hs)):
            ...
            conv_layer = LeNetConvPoolLayer(rng, input=layer0_input,image_shape=(None, 1, self.img_h, self.img_w),
                                    filter_shape=filter_shape, poolsize=pool_size, non_linear="relu", W=conv_Ws[i], b=conv_bs[i])
...
def make_idx_data_cv(revs, word_idx_map, max_l, filter_h, append_y=True):
    data = []
    for rev in revs:
...
def train_and_save(num_epochs):
    ...
    revs, W_w2v, W_rand, word_idx_map, vocab, sent_len_max = x
    train_data = make_idx_data_cv(revs, word_idx_map, sent_len_max, 5)

    model = ConvNetModel(len(train_data[0])-1, W_w2v, [None, None, None], [None, None, None], None, None)
    model.train(train_data, 50, num_epochs)  # batch size = 50

    # get fine-tuned word embeddings
    W = model.Words.get_value()

    # get params for conv layers
    conv_Ws = []
    conv_bs = []
    for conv_layer in model.conv_layers:
        conv_Ws.append(conv_layer.W.get_value())
        conv_bs.append(conv_layer.b.get_value())

    # get params for fc layer
    fc_W = model.classifier.dropout_output_layer.W.get_value()
    fc_b = model.classifier.dropout_output_layer.b.get_value()
    xlen = len(train_data[0])-1

    # store variables
    cPickle.dump([W, conv_Ws, conv_bs, fc_W, fc_b, xlen, sent_len_max, word_idx_map], open("model.p", "wb"))


def load_and_test():
    print "load the model..."
    # load params
    params = cPickle.load(open("model.p", "rb"))
    W_fine_tuned, conv_Ws, conv_bs, fc_W, fc_b, xlen = params
    ...
    # make the model
    model = ConvNetModel(xlen, W_fine_tuned, conv_Ws, conv_bs, fc_W, fc_b)
    model.train(train_data, 50, 10)


def load_model():
    params = cPickle.load(open("model.p", "rb"))
    W_fine_tuned, conv_Ws, conv_bs, fc_W, fc_b, xlen, sent_len_max, word_idx_map = params
    model = ConvNetModel(xlen, W_fine_tuned, conv_Ws, conv_bs, fc_W, fc_b)
    return model, sent_len_max, word_idx_map
...
def write_output(path, input_data, preds):
...
if __name__=="__main__":
    ...
    if input == "--train":
        ...
        model, sent_len_max, word_idx_map = load_model()
        model_input_data = make_idx_data_cv(input_data, word_idx_map, sent_len_max, 5, False)
        preds = model.predict(model_input_data)
        write_output(output_path, input_data, preds)
